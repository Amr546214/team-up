...
import { Code2, Star, Hourglass, Eye, EyeOff } from "lucide-react";
import Header from "../../../components/common/Header";
import { getCurrentUser, changePassword } from "../../../services/fakeApi";

const DevProfile = () => {
  const currentUser = getCurrentUser();
  const devProfile = currentUser?.developerProfile || {};

  const [user, setUser] = useState({
    name: currentUser?.name || "Developer",
    rank: devProfile.rank || currentUser?.quiz?.rank || "Unranked",
    title: devProfile.track ? \`\${devProfile.track} Developer\` : "Developer",
    status: "Online",
    level: devProfile.rank || "Unranked",
    experience: devProfile.experience || "Not specified",
    avatar: devProfile.image || "",
  });

  const [skills, setSkills] = useState(
    devProfile.skills?.length ? devProfile.skills : ["ui/ux", "React", "Python", "AI"]
  );
  ...
  const [rankProgress] = useState({
    currentRank: devProfile.rank || currentUser?.quiz?.rank || "Unranked",
    nextRank: "Platinum",
    currentPoints: currentUser?.quiz?.score || 0,
    targetPoints: 100,
    completedProjects: 0,
    performanceScore: 0,
  });
                ...
                {showPasswordForm && (
                  <div className="mt-5 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="h-12 w-full rounded-xl border border-gray-200 px-4 pr-12 text-sm outline-none focus:border-[#0f766e]"
                        />
                        <button
                          type="button"
                    ...
                    <button
                      ...
                      className="h-12 w-full rounded-xl bg-[#0f766e] text-sm font-medium text-white hover:bg-[#0e6d65] transition"
                    >
                      Update Password
                    </button>
                  </div>
                    ...
                    <div className="space-y-3">
                      {jobTypes.map((jobType) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={availability.preferredJobTypes.includes(jobType)}
...
fs.writeFileSync('d:/grad project/Graduation-project/src/pages/developer/dashboard/DevProfile.jsx', content);
console.log('DevProfile.jsx written successfully');
