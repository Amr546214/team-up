...
function AutoSuggestTeam() {
  ...
  const generateTeam = async () => {
    try {
      ...
      const res = await apiRequest("/ai/team-builder/recommend", {
        method: "POST",
        body: JSON.stringify({
          projectTitle: "Project Phoenix Redesign",
          teamSize: 5,
          skills: ["React", "TypeScript", "Node.js", "PostgreSQL", "Figma", "AWS"],
          budget: 10000,
          priority: "balanced",
          onlyAvailable: true,
        }),
      ...
      const newSessionId = res?.sessionId || res?.session?.sessionId || res?.id;
  ...
  const handleAcceptMember = async (developerId) => {
    if (!sessionId || !developerId) return;

    try {
      setActionLoading(true);

      await apiRequest(
        `/ai/team-builder/sessions/${sessionId}/members/${developerId}/accept`,
        { method: "PATCH" }
      );

      await loadPage(sessionId, searchQuery);
    } catch (err) {
      console.error("Accept member failed:", err);
      setError(err.message || "Failed to accept member");
    } finally {
      setActionLoading(false);
    }
  };

  const handleReplaceMember = async (developerId, preferredSkill = "React") => {
    if (!sessionId || !developerId) return;
    ...
    try {
      ...
      await apiRequest(
        `/ai/team-builder/sessions/${sessionId}/members/${developerId}/replace`,
        {
          method: "POST",
          body: JSON.stringify({ preferredSkill }),
        }
      );

      await loadPage(sessionId, searchQuery);
    } catch (err) {
      console.error("Replace member failed:", err);
      setError(err.message || "Failed to replace member");
    } finally {
      setActionLoading(false);
    }
  };
                      ...
                      <div className="space-y-3">
                        {suggestedLineup.map((member) => (
                          ...
                          >
                            <div
                              onClick={() =>
                                navigate(`/company/developers/${member.developerId}/profile`)
                              }
                            ...
                            <div className="flex flex-col gap-2 sm:items-end">
                              ...
                              {!isApproved && (
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => handleAcceptMember(member.developerId)}
                                    disabled={actionLoading || member.accepted}
                                    className="text-[11px] px-3 py-1 rounded-lg bg-[#EAF8EE] text-[#22C55E] disabled:opacity-60"
                                  >
                                    {member.accepted ? "Accepted" : "Accept"}
                                  </button>
                                  <button
                                    onClick={() =>
                                      handleReplaceMember(
                                        member.developerId,
                                        member.skills?.[0] || "React"
                                      )
                                    }
                                    disabled={actionLoading}
                                    className="text-[11px] px-3 py-1 rounded-lg bg-white border border-[#E5E7EB] text-[#6B7280] disabled:opacity-60"
                                  >
                                    Replace
                                  </button>
                                </div>
