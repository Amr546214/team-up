...
function DeveloperDashboard() {
  ...
  const statsCards = [
    { id: 1, type: "activeJobs", title: "Active jobs", value: 3, note: "+2 this week" },
    { id: 2, type: "appliedProjects", title: "Applied Projects", value: 12, note: "3 pending" },
    { id: 3, type: "totalEarning", title: "Total Earning", value: "$20.3", note: "+14%" },
    { id: 4, type: "currentRank", title: "Current Rank", value: "Gold", note: "Top 5%" },
  ];
  ...
  const applications = [
    { id: 1, type: "proposal", name: "Healthcare Booking App", budget: "$4,500", status: "pending" },
    { id: 2, type: "proposal", name: "SaaS Dashboard Redesign", budget: "$2,800", status: "accepted" },
  ];
  ...
  const apiStatsCards = dashboardData
    ? [
        {
          id: 1,
          type: "activeJobs",
          title: "Active jobs",
          value: dashboardData?.quickStats?.activeJobs ?? dashboardData?.activeJobs ?? 3,
          note: "+2 this week",
        },
        {
          id: 2,
          ...
          value: dashboardData?.quickStats?.appliedProjects ?? dashboardData?.appliedProjects ?? 12,
          note: "3 pending",
        },
        {
          id: 3,
          ...
          title: "Total Earning",
          value:
            dashboardData?.quickStats?.totalEarnings ??
            dashboardData?.totalEarningsLabel ??
        ...
        },
        {
          ...
          type: "currentRank",
          title: "Current Rank",
          value: dashboardData?.quickStats?.currentRank ?? dashboardData?.currentRank ?? "Gold",
          note: "Top 5%",
        },
      ]
    : statsCards;

  const apiActiveProjects =
    dashboardData?.activeWork?.map((project, index) => ({
      id: project.id || project.projectId || index + 1,
      name: project.projectName,
      description: project.summary,
      company: project.clientName,
      duration: project.deadline,
      status: project.statusBadge || project.status,
      progress: project.progressPercent ?? project.progress ?? 0,
    })) || activeProjects;
  ...
  const apiApplications =
    dashboardData?.applications?.map((application, index) => ({
      ...
      budget: application.proposedBudgetLabel || application.proposedBudget,
      status: application.status,
    })) || applications;

  const apiRecommendedJobs =
    dashboardData?.recommendedJobs?.map((job, index) => ({
      id: job.jobId || index + 1,
      title: job.jobTitle,
      skills: job.requiredSkills || [],
      budget: job.budgetLabel || (job.budget ? `$${job.budget.min} - $${job.budget.max}` : "Budget not specified"),
      hasApplied: job.status === "applied",
    })) || recommendedJobs;

  const apiPerformanceData = {
    averageRating: dashboardData?.performance?.averageRating ?? performanceData.averageRating,
    ratingScale: 5.0,
    completedProjects: dashboardData?.performance?.completedProjects ?? performanceData.completedProjects,
    isLoading: false,
  };
  ...
  const getStatsCardStyles = (type) => {
    switch (type) {
      case "activeJobs":
        return { icon: BriefcaseBusiness, iconColor: "text-[#22C55E]", noteColor: "text-[#22C55E]" };
      case "appliedProjects":
        return { icon: Send, iconColor: "text-[#4F7CFF]", noteColor: "text-[#9CA3AF]" };
      case "totalEarning":
        return { icon: CircleDollarSign, iconColor: "text-[#F97316]", noteColor: "text-[#22C55E]" };
      case "currentRank":
        return { icon: Star, iconColor: "text-[#EAB308]", noteColor: "text-[#9CA3AF]" };
      default:
        return { icon: BriefcaseBusiness, iconColor: "text-[#6B7280]", noteColor: "text-[#6B7280]" };
    }
  };
  ...
  const getProjectStatusStyles = (status = "") => {
    switch (status.toLowerCase()) {
      case "active":
        return { textColor: "text-[#22C55E]", dotColor: "bg-[#22C55E]", label: "Active" };
      case "planned":
        return { textColor: "text-[#EAB308]", dotColor: "bg-[#EAB308]", label: "Planned" };
      default:
        return { textColor: "text-[#6B7280]", dotColor: "bg-[#6B7280]", label: status };
    }
  };
  ...
  const getApplicationIcon = (type) => {
    switch (type) {
      case "proposal":
        return { icon: FileText, iconColor: "text-[#22C55E]" };
      default:
        return { icon: Layers3, iconColor: "text-[#22C55E]" };
    }
  };
  ...
  const getAverageRatingText = (averageRating, ratingScale) => {
    if (averageRating === null || averageRating === undefined) return "No rating yet";
    return `${averageRating}/${ratingScale}`;
  };

  const getCompletedProjectsText = (completedProjects) => {
    if (!completedProjects) return "0 Projects";
      ...
      <>
        ...
        <div className="min-h-screen bg-[#F5F9F9] mt-15 p-4 md:p-6 ml-[240px] lg:ml-[240px]">
          <div className="max-w-[1100px] mx-auto">
            <section>
              <div className="mb-6">
                <h1 className="text-[18px] md:text-[20px] font-bold text-[#111827]">
                  {typeof dashboardData?.greeting === 'string' 
                    ? dashboardData.greeting 
                    : typeof dashboardData?.greeting === 'object' && dashboardData?.greeting?.title
                      ? dashboardData.greeting.title
                      : `Hello ${userFirstName}, manage your work.`}
                </h1>
                <p className="mt-1 text-[13px] md:text-[14px] text-[#6B7280]">
                  {typeof dashboardData?.subtitle === 'string'
                    ? dashboardData.subtitle
                    : typeof dashboardData?.subtitle === 'object' && dashboardData?.subtitle?.subtitle
                      ? dashboardData.subtitle.subtitle
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <span className={`text-[12px] font-medium ${styles.noteColor}`}>{card.note}</span>
                        </div>

                        <div>
                          <h3 className="text-[28px] leading-none font-semibold text-[#111827]">{card.value}</h3>
                          <p className="mt-2 text-[14px] text-[#374151]">{card.title}</p>
                        </div>
                      </div>
                    );
                  })}
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`w-[10px] h-[10px] rounded-full ${statusStyles.dotColor}`}></span>
                          <span className={`text-[14px] font-medium ${statusStyles.textColor}`}>
                            {statusStyles.label}
                          </span>
                        </div>
                      </div>

                      <h3 className="text-[16px] font-semibold text-[#1F2937]">{project.name}</h3>
            ...
            <section className="mt-5 bg-white rounded-[14px] p-3 md:p-4">
              <h2 className="text-[18px] font-bold text-[#111827] mb-4">My Applications</h2>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="text-[16px] font-medium text-[#1F2937]">{application.name}</h3>
            ...
            <section className="mt-5 bg-white rounded-[14px] p-3 md:p-4">
              <h2 className="text-[18px] md:text-[20px] font-bold text-[#111827] mb-4">Recommended</h2>

              {apiRecommendedJobs.length === 0 ? (
                <div className="border border-[#E5E7EB] rounded-[12px] p-6 text-[14px] text-[#9CA3AF]">
                  No recommended jobs right now
                </div>
              ) : (
                <div className="space-y-4">
                  {apiRecommendedJobs.map((job) => (
                    <div
                      key={job.id}
                    ...
                    >
                      ...
                      <div className="mt-4 flex flex-wrap gap-3">
                        {job.skills?.length > 0 ? (
                          ...
                          <span className="text-[14px] text-[#9CA3AF]">No skills specified</span>
                        )}
                      </div>
            ...
            <section className="mt-5 bg-white rounded-[14px] p-3 md:p-4">
              <h2 className="text-[18px] md:text-[20px] font-bold text-[#111827] mb-4">Performance</h2>

              {apiPerformanceData.isLoading ? (
                <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="rounded-[12px] bg-[#F8FAFC] px-6 py-6 flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-[18px] md:text-[20px] text-[#9CA3AF] font-medium">Average Rating</p>
                      <h3 className="text-[20px] md:text-[24px] font-semibold text-[#111827] mt-1">
                        {getAverageRatingText(apiPerformanceData.averageRating, apiPerformanceData.ratingScale)}
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="rounded-[12px] bg-[#F8FAFC] px-6 py-6 flex items-center gap-5">
                    ...
                    <div>
                      <p className="text-[18px] md:text-[20px] text-[#9CA3AF] font-medium">Completed</p>
                      <h3 className="text-[20px] md:text-[24px] font-semibold text-[#111827] mt-1">
                        {getCompletedProjectsText(apiPerformanceData.completedProjects)}
                      </h3>
                    </div>
                  </div>
                </div>
