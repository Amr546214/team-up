...
export function ChatDevRail({
  ...
  // User search state with debounce (for Available Users panel)
  const [userSearchQuery, setUserSearchQuery] = useState('');
  const [debouncedSearchQuery, setDebouncedSearchQuery] = useState('');

  // Group member search state (for Create Group modal)
  const [groupSearchQuery, setGroupSearchQuery] = useState('');
  const [debouncedGroupSearchQuery, setDebouncedGroupSearchQuery] = useState('');
  ...
  // Debounce group member search query
  useEffect(() => {
    const timeout = setTimeout(() => {
      setDebouncedGroupSearchQuery(groupSearchQuery.trim());
    }, 300);
    return () => clearTimeout(timeout);
  }, [groupSearchQuery]);
  ...
  const handleToggleUsers = () => {
    ...
    if (nextState) {
      // Fetch users only if we have a search query of 3+ chars
  ...
  const handleOpenCreateGroup = () => {
    ...
    if (!usersPanelOpen) {
      ...
      // Only fetch if search query is 3+ chars
      if (debouncedSearchQuery.length >= 3) {
        onFetchAvailableUsers();
      }
    }
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm bg-gray-100 border-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:bg-white transition-all placeholder:text-gray-400"
              />
            </div>
            ...
            {/* Refresh button - only show when searching */}
            ...
            {/* Profiles List - only show when search has 3+ chars */}
            {debouncedSearchQuery.length >= 3 && !profilesLoading && !profilesError && (
      ...
      {createGroupOpen && (
        <div className="absolute left-14 top-0 h-full w-[280px] bg-white border-r border-gray-200 shadow-xl z-60 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
            ...
            {/* Search helper text */}
            {debouncedGroupSearchQuery.length > 0 && debouncedGroupSearchQuery.length < 3 && (
              <div className="text-xs text-gray-400 px-1">
                Type at least 3 characters to search members
              </div>
            )}

            {/* Profiles List with Checkboxes - only show when search has 3+ chars */}
            {profilesLoading && (
              <div className="flex items-center gap-2 text-xs text-gray-500 py-3">
                <div className="w-3.5 h-3.5 border-2 border-gray-300 border-t-teal-600 rounded-full animate-spin" />
                Loading users...
            ...
            {!profilesLoading && debouncedGroupSearchQuery.length >= 3 && profiles.length === 0 && (
              <p className="text-xs text-gray-500 py-2">No available users found.</p>
            )}
              ...
              <div className="space-y-1">
                {profiles
                  .filter((p) => {
                    ...
                    return (
                      query.length < 3 || // Show selected users even if search is short
                      name.includes(query) ||
                      email.includes(query) ||
                      role.includes(query) ||
                      selectedMemberIds.has(p.id) // Always show selected members
                    );
                  })
                  .slice(0, 20)
                  .map((profile) => {
